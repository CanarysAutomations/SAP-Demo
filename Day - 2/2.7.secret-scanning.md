...
1. In the GitHub repository file explorer create a test file that will contain a test secret.
    - For example the file `TailwindTraders.Website/Source/Tailwind.Traders.Web/appsettings.Development.json` with the secrets
        ```
        "StorageConnectionString":"DefaultEndpointsProtocol=https;AccountName=atailspin5546;AccountKey=pF9xY6NdSfqLlFHqWpNc5j0DwNdDviINsZqnY41+pDEnon16R+rPQykYMrt5KO8VfX+ycLd5gLfx+AStAlZEWQ==;EndpointSuffix=core.windows.net"
        ```
...
2. Add a list of paths to exclude from secret scanning. You can use [filter patterns](https://docs.github.com/en/free-pro-team@latest/actions/reference/workflow-syntax-for-github-actions#filter-pattern-cheat-sheet) to specify paths.
    ...
    **Note**: The characters `*`, `[`, and `!` are special characters in YAML. If you start a pattern with `*`, `[`, or `!`, you must enclose the pattern in quotes.
    ...
    Use a pattern to exclude the file `TailwindTraders.Website/Source/Tailwind.Traders.Web/appsettings.Development.json`
...
3. Test the pattern by adding another secret or to the file `TailwindTraders.Website/Source/Tailwind.Traders.Web/appsettings.Development.json`
